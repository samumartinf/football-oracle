...
Usage:
    python predict.py                          # All upcoming matches, normal mode
...
from models.optimizer import recommend_pick, recommend_double_match
...
def main():
    ...
    # Print per-match predictions
    header = (
        f"{'Match':<30} {'Pick':>10} {'Score':>6} {'EV':>8} {'Total':>8} {'Bonus':>14}"
    )
    print("\n" + header)
    print("-" * len(header))
    ...
    for match in MPP_MATCHES:
        ...
        print(
            ...
            f"{pick['ev']:>7.1f}  {pick['total_possible']:>5}   {bonus_str:<14}"
        )
